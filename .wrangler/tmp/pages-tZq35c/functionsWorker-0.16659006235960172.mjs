...
// ../.wrangler/tmp/bundle-9EhPZG/checked-fetch.js
  ...
  try {
    ...
    if (Array.isArray(data)) {
      const statements = data.map((item) => {
        return env.DB.prepare("INSERT INTO users (username, password, role, status_voting) VALUES (?, ?, 'pemilih', 0)").bind(item.username, item.password);
      });
...
async function onRequestPost4(context) {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      const statements = data.map((item) => {
        return env.DB.prepare("INSERT INTO kandidat (nama, visi_misi, total_suara) VALUES (?, ?, 0)").bind(item.nama, item.visi_misi);
      });
      await env.DB.batch(statements);
      return new Response(JSON.stringify({ success: true, message: "Bulk import berhasil" }), { status: 200 });
    } else {
...
// ../.wrangler/tmp/pages-tZq35c/functionsRoutes-0.3821902730462374.mjs
var routes = [
  {
    routePath: "/api/dpt",
    mountPath: "/api",
...
// C:/Users/LENOVO/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// C:/Users/LENOVO/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
...
// ../.wrangler/tmp/bundle-9EhPZG/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-9EhPZG/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.16659006235960172.mjs.map
